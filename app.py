...
# Custom header
st.markdown("<div class='title-text'>🎵 AcousticID</div>", unsafe_allow_html=True)
...
with tab_identify:
    ...
    with col_input:
        input_type = st.radio("Choose Input Method", ["Upload Audio File", "Select Sample Song from DB Folder (Simulated Query)"])
        
        uploaded_file = None
        sample_filename = None
        
        if input_type == "Upload Audio File":
            uploaded_file = st.file_uploader("Upload query song (.mp3 or .wav)", type=["mp3", "wav"])
        else:
            # List actual .mp3 files in the DB directory
            if os.path.exists(DB_DIR):
                mp3_files = [f for f in os.listdir(DB_DIR) if f.endswith(".mp3")]
                if mp3_files:
                    sample_filename = st.selectbox("Select song to query:", sorted(mp3_files))
                else:
    ...
    if st.button("Identify Song", type="primary", use_container_width=True):
        ...
        if input_type == "Upload Audio File" and uploaded_file is not None:
            with st.spinner("Decoding uploaded audio..."):
                try:
                    # Load and crop
                    audio_data = load_audio_from_uploaded(uploaded_file, limit=limit_seconds)
            ...
            with st.spinner(f"Loading {sample_filename}..."):
                try:
                    ...
                    cropped_audio = audiofile[offset_seconds*1000 : (offset_seconds + limit_seconds)*1000]
        ...
        if audio_data:
            ...
            with st.spinner("Extracting spectrogram & generating peaks..."):
                channel_samples = audio_data['channels'][0] # Use channel 0 for analysis
                Fs = audio_data['Fs']
            ...
            if alignment:
                ...
                with col_spec:
                    ...
                    im = ax.imshow(arr2D, aspect='auto', origin='lower', cmap='viridis')
                    # Overlay peaks
                    freqs = [p[0] for p in peaks]
                    times = [p[1] for p in peaks]
                    ax.scatter(times, freqs, color='red', s=4, label='Constellation Peaks', alpha=0.8)
                ...
                with col_hist:
                    ...
                    ax2.hist(winner_offsets, bins=100, color='#8A2BE2', edgecolor='black', alpha=0.8)
                ...
                if not cand_df.empty:
                    ...
                    st.dataframe(cand_df[["Song Name", "Spike Confidence", "Total Matches"]], use_container_width=True)
            else:
                st.warning("❌ No matching song found. Try increasing the duration or querying a different part of the audio.")
...
with tab_db:
    ...
    avg_fingerprints = int(total_fingerprints / total_songs) if total_songs > 0 else 0
            ...
            if not os.path.exists(DB_DIR):
                ...
                if not mp3_files:
                    ...
                    for idx, filename in enumerate(mp3_files):
                        status_text.text(f"Processing ({idx+1}/{len(mp3_files)}): {filename}")
                        ...
                        try:
                            ...
                            if existing_song:
                                hash_count = db.get_song_hashes_count(existing_song[0])
                                if hash_count > 0:
                                    skipped_count += 1
                            ...
                            for channeln, channel in enumerate(audio['channels']):
                                arr2D = core.get_spectrogram(channel, Fs=audio['Fs'])
                                peaks = core.get_2D_peaks(arr2D)
                                hashes |= set(core.generate_hashes(peaks))
                            
                            # 3. Store in DB
                            fingerprints = [(song_id, h, offset) for h, offset in hashes]
                    ...
                    status_text.text(f"Indexing Complete! Stored {added_count} new songs, skipped {skipped_count} existing songs.")
                    st.balloons()
                    time.sleep(1)
                    st.rerun()
    ...
    if all_songs:
        song_df = pd.DataFrame(all_songs, columns=["ID", "Filename", "File Hash (SHA1)", "Fingerprint Count"])
        st.dataframe(song_df, use_container_width=True, hide_index=True)
        
        # Individual song deletion
        st.subheader("Delete Song from Index")
        delete_song_name = st.selectbox("Select song to delete:", [s[1] for s in all_songs])
        if st.button("Delete Selected Song"):
            selected_song = next(s for s in all_songs if s[1] == delete_song_name)
            db.delete_song(selected_song[0])
            st.success(f"Deleted '{delete_song_name}' from the index.")
            time.sleep(1)
            st.rerun()
    else:
        st.info("Database is empty. Click 'Scan & Index DB Folder' above to search for songs.")
...
with tab_batch:
    ...
    uploaded_files = st.file_uploader("Upload query files (.mp3 or .wav)", type=["mp3", "wav"], accept_multiple_files=True)
    
    if uploaded_files:
        st.write(f"Loaded {len(uploaded_files)} files. Press run below to process them.")
        ...
        if st.button("Run Batch Identification", type="primary"):
            ...
            for idx, file_obj in enumerate(uploaded_files):
                status_text.text(f"Identifying ({idx+1}/{len(uploaded_files)}): {file_obj.name}")
                
                try:
                    # Load audio
                    audio_data = load_audio_from_uploaded(file_obj, limit=limit_seconds)
                    ...
                    if alignment:
                        results.append({
                            ...
                            "Predicted Song": alignment['song_name'],
                            "Spike Confidence": alignment['confidence'],
                            "Offset (seconds)": alignment['offset_secs']
                        })
                    else:
                        results.append({
                            "File Name": file_obj.name,
                            "Predicted Song": "No Match Found",
                            "Spike Confidence": 0,
                            "Offset (seconds)": 0.0
                        })
                except Exception as e:
